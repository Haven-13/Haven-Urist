...
/mob/Stat()
	...
	if(statpanel("Status"))
		if(GAME_STATE >= RUNLEVEL_LOBBY)
			...
			stat("Server Date", time_stamp("YYYY-MM-DD"))
	...
	if(client.holder)
		if(statpanel("MC"))
			stat("CPU:","[world.cpu]")
			stat("Maptick:", "[world.map_cpu] ([world.map_cpu / length(GLOB.clients)] per player)")
...
/mob/proc/yank_out_object()
	...
	if(is_human_mob(src))
		...
		H.shock_stage+=20
		affected.take_external_damage((selection.w_class * 3), 0, DAMAGE_FLAGS_EDGE, "Embedded object extraction")
