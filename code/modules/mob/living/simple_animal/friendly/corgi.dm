		...
		if(turns_since_scan > 5)
			...
			if( !movement_target || !(movement_target.loc in oview(src, 3)) )
				...
				for(var/obj/item/weapon/reagent_containers/food/snacks/S in oview(src,3))
					if(is_turf(S.loc) || is_human_mob(S.loc))
						movement_target = S
						break
			if(movement_target)
				...
				if(movement_target)		//Not redundant due to sleeps, Item can be gone in 6 decisecomds
					...
					if(is_turf(movement_target.loc) )
						UnarmedAttack(movement_target)
					else if(is_human_mob(movement_target.loc) && prob(20))
...
/mob/living/simple_animal/corgi/attackby(obj/item/O as obj, mob/user as mob)
...
/mob/living/simple_animal/corgi/regenerate_icons()
	...
	if(inventory_head)
		...
		var/icon/head_icon = image('resources/icons/mob/corgi_head.dmi',head_icon_state)
		if(head_icon)
			overlays += head_icon
	...
	if(inventory_back)
		...
		var/icon/back_icon = image('resources/icons/mob/corgi_back.dmi',back_icon_state)
		if(back_icon)
			overlays += back_icon
	return
