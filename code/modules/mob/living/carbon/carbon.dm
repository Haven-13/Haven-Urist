	...
	if((user in src.stomach_contents) && istype(user))
		if(user.last_special <= world.time)
			...
			if(I && I.force)
				...
				playsound(user.loc, 'resources/sound/effects/attackblob.ogg', 50, 1)
...
/mob/living/carbon/proc/apply_shock(shock_damage, def_zone, siemens_coeff = 1.0)
	...
	if(shock_damage < 1)
		shock_damage = 1
	apply_damage(shock_damage, DAMAGE_TYPE_BURN, def_zone, used_weapon="Electrocution")
	return(shock_damage)
...
/mob/living/carbon/proc/activate_hand(selhand)
...
/mob/living/carbon/proc/help_shake_act(mob/living/carbon/M)
	if(!is_asystole())
		if (on_fire)
			playsound(src.loc, 'resources/sound/weapons/thudswoosh.ogg', 50, 1, -1)
			if (M.on_fire)
...
/mob/living/carbon/show_inv(mob/user as mob)
	...
	<BR><HR>
	<BR><B>Head(Mask):</B> <A href='?src=[REF(src)];item=mask'>[(wear_mask || "Nothing")]</A>
	<BR><B>Left Hand:</B> <A href='?src=[REF(src)];item=l_hand'>[(l_hand || "Nothing")]</A>
	<BR><B>Right Hand:</B> <A href='?src=[REF(src)];item=r_hand'>[(r_hand || "Nothing")]</A>
	<BR><B>Back:</B> <A href='?src=[REF(src)];item=back'>[(back || "Nothing")]</A> [((istype(wear_mask, /obj/item/clothing/mask) && istype(back, /obj/item/weapon/tank) && !( internal )) ? text(" <A href='?src=\ref[];item=internal'>Set Internal</A>", src) : "")]
