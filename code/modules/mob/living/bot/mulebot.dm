...
/mob/living/bot/mulebot/proc/runOver(mob/living/carbon/human/H)
	if(istype(H)) // No safety checks - WILL run over lying humans. Stop ERPing in the maint!
		...
		H.apply_damage(2 * damage, DAMAGE_TYPE_BRUTE, BP_CHEST)
		H.apply_damage(0.5 * damage, DAMAGE_TYPE_BRUTE, BP_L_LEG)
		H.apply_damage(0.5 * damage, DAMAGE_TYPE_BRUTE, BP_R_LEG)
		H.apply_damage(0.5 * damage, DAMAGE_TYPE_BRUTE, BP_L_ARM)
		H.apply_damage(0.5 * damage, DAMAGE_TYPE_BRUTE, BP_R_ARM)
...
/mob/living/bot/mulebot/proc/load(atom/movable/C)
	if(busy || load || get_dist(C, src) > 1 || !is_turf(C.loc))
		return
