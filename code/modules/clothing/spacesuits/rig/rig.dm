...
/obj/item/weapon/rig
	...
	var/equipment_overlay_icon = 'resources/icons/mob/onmob/rig_modules.dmi'
...
/obj/item/weapon/rig/Initialize()
	...
	for(var/obj/item/piece in list(gloves,helmet,boots,chest))
		...
		if(is_list(armor)) piece.armor = armor.Copy()
...
/obj/item/weapon/rig/ui_interact(mob/user, datum/tgui/ui)
	if(!user)
		return

	ui = SStgui.try_update_ui(user, src, ui)
	if (!ui)
		ui = new(user, src, ((src.loc != user) ? ai_interface_path : interface_path))
		ui.open()
...
/obj/item/weapon/rig/update_icon(update_mob_icon)
	...
	if(!mob_icon || update_mob_icon)
		var/species_icon = 'resources/icons/mob/onmob/rig_back.dmi'
