...
/dmm_suite/proc/readlist(text as text, delimiter=",")
	...
	do
		...
		var/trim_left = trim_text(copytext(text,old_position,(equal_position || position)),1)//the name of the variable, must trim quotes to build a BYOND compliant associatives list
