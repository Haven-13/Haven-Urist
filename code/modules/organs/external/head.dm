/obj/item/organ/external/head
	...
	var/eye_icon_location = 'resources/icons/mob/human_races/species/default_eyes.dmi'
	var/has_lips = 1
...
/obj/item/organ/external/head/proc/get_eye_cache_key()
	...
	if(owner)
		var/obj/item/organ/internal/eyes/eyes = owner.internal_organs_by_name[owner.species.vision_organ || BP_EYES]
...
/obj/item/organ/external/head/update_icon()
	...
	if(owner)
		...
		if(owner.lip_style && !BP_IS_ROBOTIC(src) && (species && (species.appearance_flags & HAS_LIPS)))
			var/icon/lip_icon = new/icon('resources/icons/mob/human_races/species/human/lips.dmi', "lips_[owner.lip_style]_s")
			overlays |= lip_icon
...
/obj/item/organ/external/head/proc/get_hair_icon()
	...
	if(owner.h_style)
		...
		if(hair_style && (species.get_bodytype(owner) in hair_style.species_allowed))
			...
			if(hair_style.do_colouration && is_list(h_col) && h_col.len >= 3)
