...
/datum/seed/proc/do_thorns(var/mob/living/carbon/human/target, var/obj/item/fruit, var/target_limb)
	...
	var/damage_flags = DAMAGE_FLAGS_SHARP|(has_edge? DAMAGE_FLAGS_EDGE : 0)
	target.apply_damage(damage, DAMAGE_TYPE_BRUTE, target_limb, blocked, damage_flags, "Thorns")
...
/datum/seed/proc/get_icon(growth_stage)
	...
	var/image/res = image('resources/icons/obj/hydroponics_growing.dmi', "[plant_icon]-[growth_stage]")
	if(get_growth_type())
	...
	if(get_growth_type())
		res.icon = 'resources/icons/obj/hydroponics_vines.dmi'

	res.color = get_trait(TRAIT_PLANT_COLOUR)

	if(get_trait(TRAIT_LARGE))
		res.icon = 'resources/icons/obj/hydroponics_large.dmi'
		res.pixel_x = -8
		res.pixel_y = -16
