...
/datum/controller/plants/proc/setup()
	...
	for(var/icostate in icon_states('resources/icons/obj/hydroponics_growing.dmi'))
	...
	for(var/icostate in icon_states('resources/icons/obj/hydroponics_products.dmi'))
		var/split = findtext(icostate,"-")
		if(split)
