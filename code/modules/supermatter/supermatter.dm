		...
		if((damage > emergency_point) && !public_alert)
			...
			for(var/mob/M in GLOB.player_list)
				...
				if(T && (T.z in GLOB.using_map.station_levels) && !istype(M,/mob/new_player) && !isdeaf(M))
					sound_to(M, 'resources/sound/ambience/matteralarm.ogg')
...
/obj/machinery/power/supermatter/Process()
	...
	if(!env || !removed || !removed.total_moles)
		...
		removed.adjust_multi(
			"phoron",
			max(device_energy / phoron_release_modifier, 0),
			"oxygen",
			max((device_energy + removed.temperature - T0C) / oxygen_release_modifier, 0)
		)
