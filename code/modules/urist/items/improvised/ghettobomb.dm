...
/obj/item/weapon/reagent_containers/food/drinks/cans/attackby(obj/item/I, mob/user as mob)
...
/obj/item/weapon/grenade/iedcasing/afterattack(atom/target, mob/user , flag)
	if(assembled == 0)
		if(istype(target, /obj/structure/reagent_dispensers/fueltank) && in_range(src, target))
			...
			playsound(get_turf(src), 'resources/sound/effects/refill.ogg', 50, 1, -6)
			desc = "An improvised explosive assembly. Filled to the brim with 'Explosive flavor'"
			overlays += image('resources/icons/obj/grenade.dmi', icon_state = "improvised_grenade_filled")
			return


/obj/item/weapon/grenade/iedcasing/attackby(obj/item/I, mob/user as mob)
	if(istype(I, /obj/item/stack/cable_coil))
		if(assembled == 1)
			...
			overlays += image('resources/icons/obj/grenade.dmi', icon_state = "improvised_grenade_wired")
