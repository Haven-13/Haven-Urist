...
/mob/living/simple_animal/hostile/npc/proc/generate_trade_items()
	...
	for(var/category in trade_categories_by_name)
		var/datum/trade_category/C = SStrade_controller.get_trade_category(category)
		if(C)
		...
		for(var/datum/trade_item/I in trade_items_other)
			...
			if(index_weight >= target_weight)
				...
				for(var/trade_type in I.bonus_items)
					var/datum/trade_item/bonus_item = SStrade_controller.trade_items_by_type[trade_type]
...
/mob/living/simple_animal/hostile/npc/proc/spawn_trade_item(datum/trade_item/I, hidden = 0)
	if(!I)
		return
...
/mob/living/simple_animal/hostile/npc/proc/generate_trade_item_ui(datum/trade_item/T)
	if(T)
		interact_inventory.Add(list(list("name" = T.name, "quantity" = T.quantity, "value" = T.value)))

/mob/living/simple_animal/hostile/npc/proc/update_trade_item_ui(datum/trade_item/T)
	if(T)
		var/found = 0
