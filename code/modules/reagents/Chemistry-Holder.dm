...
/datum/reagents/proc/handle_update(safety = FALSE)
	update_total()
	if(!safety)
		QUEUE_REACTIONS(src)
	if(my_atom)
...
/datum/reagents/proc/trans_to_holder(datum/reagents/target, amount = 1, multiplier = 1, copy = 0, safety = 0) // Transfers [amount] reagents from [src] to [target], multiplying them by [multiplier]. Returns actual amount removed from [src] (not amount transferred to [target])
...
/datum/reagents/proc/trans_to_turf(turf/target, amount = 1, multiplier = 1, copy = 0) // Turfs don't have any reagents (at least, for now)
