...
/obj/structure/artilleryplaceholder
	name = "artillery"
	icon = 'resources/icons/obj/machines/artillery.dmi'
	anchored = 1
	density = 1
...
/obj/machinery/artillerycontrol/Topic(href, href_list, state = ui_physical_state())
	if(..())
		return 1
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if (!thearea || CanUseTopic(usr, ui_physical_state()) != UI_INTERACTIVE)
			return
		if (src.reload < 180)
			return
		if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
