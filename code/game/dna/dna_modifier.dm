...
/obj/machinery/dna_scannernew/Initialize()
	. = ..()
	build_default_parts(/obj/item/weapon/circuitboard/clonescanner)
...
/obj/machinery/computer/scan_consolenew/ui_data(mob/user)
	...
	data["beakerVolume"] = 0
	if(connected.beaker)
		data["beakerLabel"] = connected.beaker.label_text || null
...
/obj/machinery/computer/scan_consolenew/Topic(href, href_list)
	...
	if (href_list["pulseRadiation"])
		...
		src.connected.occupant.apply_effect(((src.radiation_intensity*3)+src.radiation_duration*3), DAMAGE_TYPE_RADIATION, blocked = 0)
	...
	if (href_list["pulseUIRadiation"])
		...
		if (prob((80 + (src.radiation_duration / 2))))
			...
			src.connected.occupant.apply_effect((src.radiation_intensity+src.radiation_duration), DAMAGE_TYPE_RADIATION, blocked = 0)
		else
			if	(prob(20+src.radiation_intensity))
				...
				randmuti(src.connected.occupant)
				src.connected.occupant.UpdateAppearance()
			src.connected.occupant.apply_effect(((src.radiation_intensity*2)+src.radiation_duration), DAMAGE_TYPE_RADIATION, blocked = 0)
		src.connected.locked = lock_state
		return 1 // return 1 forces an update to all Nano uis attached to src
	...
	if(href_list["bufferOption"])
		...
		if (bufferOption == "transfer")
			...
			if ((buf.types & DNA2_BUF_UI))
				...
				domutcheck(src.connected.occupant,src.connected)
			src.connected.occupant.apply_effect(rand(20,50), DAMAGE_TYPE_RADIATION, blocked = 0)
			return 1
