/obj/machinery/cooker/fryer
	...
	off_icon = "fryer_off"
	food_color = "#ffad33"
	cooked_sound = 'resources/sound/machines/ding.ogg'
...
/obj/machinery/cooker/fryer/cook_mob(var/mob/living/victim, var/mob/user)
	...
	if(ishuman(victim) && !(target_zone in list(BP_GROIN, BP_CHEST)))
		...
		if(!E)
			...
			H.apply_damage(rand(20,30), DAMAGE_TYPE_BURN, target_zone, blocked)

	else
		var/blocked = victim.run_armor_check(null, "energy")
		victim.apply_damage(rand(30,40), DAMAGE_TYPE_BURN, null, blocked)
