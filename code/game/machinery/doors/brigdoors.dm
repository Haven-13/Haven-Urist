...
// #define FONT_SIZE "5pt" // already defined somewhere else with the same value
...
/obj/machinery/door_timer/proc/set_picture(state)
	picture_state = state
	overlays.Cut()
	overlays += image('resources/icons/obj/status_display.dmi', icon_state=picture_state)
...
/obj/machinery/door_timer/proc/texticon(tn, px = 0, py = 0)
	var/image/I = image('resources/icons/obj/status_display.dmi', "blank")
	var/len = length(tn)
	...
	for(var/d = 1 to len)
		...
		if(char == " ")
			continue
		var/image/ID = image('resources/icons/obj/status_display.dmi', icon_state=char)
		ID.pixel_x = -(d-1)*5 + px
		ID.pixel_y = py
		I.overlays += ID
	return I
