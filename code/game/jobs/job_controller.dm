...
**/
/datum/controller/occupations/proc/DivideOccupations(datum/game_mode/mode)
...
/datum/controller/occupations/proc/EquipRank(mob/living/carbon/human/H, rank, joined_late = 0)
	...
	to_chat(H, "<B>You are [job.total_positions == 1 ? "the" : "a"] [alt_title || rank].</B>")

	if(job.supervisors)
		to_chat(H, "<b>As the [alt_title || rank] you answer directly to [job.supervisors]. Special circumstances may change this.</b>")
