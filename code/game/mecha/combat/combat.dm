...
/obj/mecha/combat/melee_action(target as obj|mob|turf)
	...
	if(istype(target, /mob/living))
		...
		if(src.occupant.a_intent == I_HURT)
			playsound(src, 'resources/sound/weapons/punch4.ogg', 50, 1)
			if(damtype == "brute")
				step_away(M,src,15)
			/*
			if(M.stat>1)
			...
			*/
			...
			switch(damtype)
				if("brute")
					...
					if(M.apply_damage(rand(force/2, force), DAMAGE_TYPE_BRUTE, hit_zone, blocked))
						M.Weaken(1)
				if("fire")
					var/blocked = M.run_armor_check(hit_zone, "energy")
					M.apply_damage(rand(force/2, force), DAMAGE_TYPE_BRUTE, hit_zone, blocked)
				if("tox")
					if(M.reagents)
