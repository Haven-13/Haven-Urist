...
/obj/item/weapon/soap/Crossed(mob/living/AM)
	if (istype(AM))
		if(AM.pulledby)
			return
...
/obj/item/weapon/soap/attack(mob/living/target, mob/living/user, target_zone)
	if(target && user && is_human_mob(target) && is_human_mob(user) && !target.stat && !user.stat && user.zone_sel &&user.zone_sel.selecting == BP_MOUTH)
...
/obj/item/weapon/soap/update_icon()
	overlays.Cut()
	if(key_data)
		overlays += image('resources/icons/obj/items.dmi', icon_state = "soap_key_overlay")
