...
/obj/item/device/camera/tvcamera/attack_self(mob/user)
	...
	dat += "Channel name is: <a href='?src=\ref[src];channel=1'>[channel || "unidentified broadcast"]</a><br>"
...
/obj/item/device/camera/tvcamera/Topic(bred, href_list, state = ui_physical_state())
	if(..())
		return 1
	if (href_list["photo"])
		on = !on
