...
/obj/item/proc/zoom(mob/user, tileoffset = 14,viewsize = 9)
	if(!user.client)
		return
	if(zoom)
		return
	...
	GLOB.moved_event.register(src, src, TYPE_PROC_REF(/obj/item, unzoom))
	GLOB.dir_set_event.register(src, src, TYPE_PROC_REF(/obj/item, unzoom))
	GLOB.item_unequipped_event.register(src, src, TYPE_PROC_REF(/obj/item, zoom_drop))
...
/obj/item/proc/unzoom(mob/user)
	...
	GLOB.moved_event.unregister(src, src, TYPE_PROC_REF(/obj/item, unzoom))
	GLOB.dir_set_event.unregister(src, src, TYPE_PROC_REF(/obj/item, unzoom))
	GLOB.item_unequipped_event.unregister(src, src, TYPE_PROC_REF(/obj/item, zoom_drop))
