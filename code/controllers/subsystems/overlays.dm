...
/atom/var/list/add_overlays
/atom/var/list/remove_overlays
...
/atom/proc/cut_overlay(list/overlays)
	...
	if(NOT_QUEUED_ALREADY && (fa_len != a_len || fr_len != r_len ))
		QUEUE_FOR_COMPILE
	UNSET_EMPTY(add_overlays)

/atom/proc/add_overlay(list/overlays)
	if(!overlays || !length(overlays))
		return

	overlays = build_appearance_list(overlays)

	LAZY_INIT(add_overlays) //always initialized after this point
	var/a_len = add_overlays.len
...
/atom/proc/copy_overlays(atom/other, cut_old) //copys our_overlays from another atom
	...
	if(cached_other)
		if(cut_old || !LAZY_LENGTH(overlays))
			remove_overlays = overlays
