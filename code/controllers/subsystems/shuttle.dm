...
/datum/controller/subsystem/shuttle/Initialize()
	...
	for(var/shuttle_type in subtypesof(/datum/shuttle)) // This accounts for most shuttles, though away maps can queue up more.
		...
		if(!initial(shuttle.defer_initialisation))
			LAZY_ADD_UNIQUE(shuttles_to_initialize, shuttle_type)
...
/datum/controller/subsystem/shuttle/proc/initialize_sector(obj/effect/overmap/given_sector)
	// This is a late init operation that sets up the sector's map_z and
	// does non-overmap-related init tasks.
	given_sector.populate_sector_objects()
