/datum/map/tyclo_pluto/map_info(victim)
	...
	to_chat(victim, "You're aboard the <b>[src.name]</b>, an independently contracted vessel owned by the Captain. Its primary mission is whatever the Captain dictates, which can include trading, scavenging or exploration.")
...
/datum/map/tyclo_pluto/send_welcome()
	var/welcome_text = "<center><br /><font size = 3><b>[src.name]</b> Sensor Readings:</font><hr />"
	...
	post_comm_message("[src.name] Sensor Readings", welcome_text)
